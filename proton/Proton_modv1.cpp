/*
   ...
   */
/*
   - A. Pérez de Alba Ortíz, A. Tiwari, R. C. Puthenkalathil, and B. Ensing , "Advances in enhanced sampling along adaptive paths of collective variables", J. Chem. Phys. 149, 072320 (2018) https://doi.org/10.1063/1.5027392
   - https://www.compchem.nl/software_package/pka-calculation-with-a-proton-transfer-collective-variable/
   - https://www.acmm.nl/molsim/users/ensing/software/ptcv_plumednest.zip
   */
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
*/
        ...
        void Proton::calculate(){
            ...
            std::vector<Vector> derivtmp(distref);
            for(unsigned l=0;l<no;l++){
                for(unsigned m=0;m<3;m++){
                    derivtmp[l][m] = 0.0;
                    for(unsigned j=0;j<no;j++) derivtmp[l][m] += distref[j][m] * derivw[j][l];
                }
            }
            for(unsigned i=0;i<getNumberOfAtoms();i++){
                for(unsigned l=0;l<no;l++){
                    for(unsigned k=0;k<3;k++){
                        for(unsigned m=0;m<3;m++) deriv[i][k][m] += derivtmp[l][m] * derivcn[l][i][k];
                    }
                }
            }
...
/*
  ...
  for(unsigned i=0; i<getNumberOfAtoms(); ++i) {
      log.printf((" "+fmt_xyz+" "+fmt_xyz+" "+fmt_xyz+" "+fmt_xyz+" "+fmt_xyz+" "+fmt_xyz+" "+fmt_xyz+" "+fmt_xyz+" "+fmt_xyz+"\n").c_str(),
        deriv[i][0][0],deriv[i][0][1],deriv[i][0][2],
        deriv[i][1][0],deriv[i][1][1],deriv[i][1][2],
        deriv[i][2][0],deriv[i][2][1],deriv[i][2][2]
      );
  };
*/
