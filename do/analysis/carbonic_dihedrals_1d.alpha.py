...
def run(
    ...
    df_data = df_data[df_data['dihedral1(rad)'] < np.pi/2]
    print(df_data)
    ser_sum = df_data['dihedral0(rad)']
    ...
    ax.set_xlabel(r'$\alpha [\beta<\pi/2]$ (rad)')
    ...
    df.to_csv('carbonic_dihedrals_1d.alpha.csv', index=False)
...
def main():
    ...
    plot.save(
        fig,
        file_save = 'carbonic_dihedrals_1d.alpha',
        list_type = ['pdf', 'svg']
    )
